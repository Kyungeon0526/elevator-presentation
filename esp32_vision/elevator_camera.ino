...
// ============================================================
// 카메라 모델에 따른 핀 맵 (ESP32-S3OV3660)
// ============================================================
// ESP32-S3OV3660 모델 - OV3660 카메라 핀 설정
...
void initCamera() {
    ...
    config.pixel_format = PIXFORMAT_GRAYSCALE; // 그레이스케일 사용
    ...
    if (err != ESP_OK) {
        ...
        Serial.println("카메라 모델을 감지하지 못했습니다. 핀 설정을 확인하세요.");
        return;
    }
...
void handleCapture() {
    ...
    server.sendHeader("Content-Type", "image/jpeg; charset=UTF-8");
...
void setup() {
    ...
    // 웹서버 기본 헤더 설정 (UTF-8 인코딩)
    server.enableCORS(true);  // CORS 활성화
